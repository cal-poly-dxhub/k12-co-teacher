...
import { getStudentsByClassId, getClassesForDashboard } from "../../../../../lib/api";
import { studentCache } from "../../../../../lib/studentCache";
import { useWebSocket } from "../../../../../hooks/useWebSocket";
import ReactMarkdown from "react-markdown";

export default function StudentChatPageClient() {
  const auth = useAuth();
  const params = useParams();
  const router = useRouter();
  const [message, setMessage] = useState("");
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [classInfo, setClassInfo] = useState<{title: string, section: string} | null>(null);
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && (
              <div className="flex justify-center items-center h-full">
                <p className="text-gray-400 text-lg">Welcome to a new student chat!</p>
              </div>
            )}
            {messages.map((msg) => (
